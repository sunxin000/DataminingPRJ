...
def index():
    ...
    movie_info_input = pd.DataFrame(columns=[
        "title", "overview", "image_url", "popularity", "release_date",
        "video_url"
    ])
    # Get info from TMDB
...
def recommender():
    ...
    for label, rating in user_input.items():
        ...
        input_frame = input_frame.append(agg, ignore_index=True)
    ...
    if len_ratings > 5:
        rec = nmf_recommand(model=nmf,
                            new_user=input_frame,
                            n=5,
                            orig_data=ratings_pivot,
                            cols_above=cols_above,
                            cols_below=cols_below,
                            selection=int(selection))

    else:
        sim_matrix = calculate_similarity_matrix(input_frame,
                                                 orig_data=ratings_pivot,
                                                 n_users=3)
        ...
        rec = collaborative_filtering(rec_for_sim_users,
                                      n=5,
                                      new_user_input=input_frame)
